import json
import re
from typing import List
from langchain_core.messages import ToolCall




def makeJSONToToolCall(content_str: str) -> List[ToolCall]:
    """
    LLM이 생성한 문자열에서 <tool_call> 태그를 찾아 JSON으로 파싱합니다.
    """
    tool_calls: List[ToolCall] = []
    pattern = r"<tool_call>(.*?)</tool_call>"
    matches = re.findall(pattern, content_str, re.DOTALL)

    for match in matches:
        json_str = match.strip()
        data = None
        try:
            data = json.loads(json_str)
        except json.JSONDecodeError:
            # 간단한 JSON 오류 수정 시도
            fixed_json_str = None
            if json_str.endswith("}}"):
                fixed_json_str = json_str[:-1]
            elif json_str.startswith("{") and not json_str.endswith("}"):
                fixed_json_str = json_str + "}"
            
            if fixed_json_str:
                try:
                    data = json.loads(fixed_json_str)
                except json.JSONDecodeError as e2:
                    print(f"ERROR: Failed to parse even after fixing. Block: {json_str}\nError: {e2}")
            else:
                print(f"ERROR: Cannot fix JSON with simple heuristics. Block: {json_str}")

        if data:
            name = data.get("name")
            args = data.get("arguments")
            if name and args is not None:
                tool_call = ToolCall(name=name, args=args, id=f"tool_{name}_{len(tool_calls)}")
                tool_calls.append(tool_call)
        else:
            # 파싱 실패 시 fallback
            tool_call = ToolCall(name="retrieve", args={"query": "__NONE__"}, id=f"tool_fallback_{len(tool_calls)}")
            tool_calls.append(tool_call)

    return tool_calls